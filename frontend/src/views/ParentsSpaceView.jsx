          ...
          <div className="bg-[#F2FDF6] rounded-3xl p-4.5 shadow-xs border-2 border-[#BBF7D0] space-y-3">
              ...
              {lucasItems.length === 0 ? (
                ...
                lucasItems.map(item => (
                  <div key={item.id} className="p-2.5 rounded-2xl bg-white/90 border border-[#BBF7D0]/70 flex items-center justify-between shadow-2xs hover:bg-white transition-all">
                    ...
                    >
                      <div className={`w-5 h-5 rounded-lg border flex items-center justify-center ${
                        item.completed ? 'bg-[#1E4E42] border-[#166534] text-white' : 'border-slate-300 bg-white'
                      }`}>
                        {item.completed && <CheckCircle2 className="w-3.5 h-3.5" />}
                      </div>
                      <span className={item.completed ? 'line-through text-slate-400 font-medium' : 'text-slate-800 font-bold'}>
            ...
            <form onSubmit={(e) => handleAddPersonItem(e, 'lucas', inlineNewLucas, setInlineNewLucas)} className="flex items-center gap-2 pt-1">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-xl border border-[#BBF7D0] text-xs bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#1E4E42]"
              />
              <button
                type="submit"
                className="w-8 h-8 rounded-xl bg-[#1E4E42] text-white flex items-center justify-center shadow-xs cursor-pointer active:scale-95 flex-shrink-0"
                title="Ajouter"
              >
                <Plus className="w-4 h-4 stroke-[3px]" />
              </button>
                  ...
                  {uncompletedRdvs.length > 1 && (
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        className="w-6 h-6 rounded-full bg-amber-50 border border-[#EFE89F] flex items-center justify-center text-[#D26E7B] disabled:opacity-30 cursor-pointer"
                        title="Rendez-vous précédent"
                      >
                        <ChevronLeft className="w-3.5 h-3.5" />
                      </button>
                      <span className="text-[10px] font-black text-[#D26E7B] px-1">
                        ...
                        <div className="flex items-start gap-3.5 px-1">
                          ...
                          <div className="space-y-2 flex-1 min-w-0">
                            ...
                            {isCurrentActive && rdv.notes && (
                              <p className="text-[11px] text-slate-600 italic bg-white/80 rounded-xl p-2.5 border border-[#EFE89F]/70 break-words leading-relaxed animate-in fade-in-50">
                                « {rdv.notes} »
                              </p>
                            )}
                          </div>
